...
/**
 * Calculates contrast color (black or white) based on a hex color.
 */
...
function useParticles(canvasRef, particleColor) {
  ...
  useEffect(() => {
    ...
    const P = Array.from({ length: 30 }, () => ({
      ...
      vx: (Math.random() - 0.5) * 0.25, vy: -Math.random() * 0.28 - 0.08,
      r: Math.random() * 2 + 0.5, a: Math.random() * 0.35 + 0.06,
...
const Splash = memo(() => (
  <div className="splash" aria-hidden="true">
    <div className="splash-logo">
      ...
      <span className="splash-title">OPTIQ</span>
    </div>
    <p className="splash-sub">Recycled eyewear · 3D printed for you</p>
...
export default function App() {
  ...
  const displayToast = useCallback((msg) => { setToastMsg(msg); setShowToast(true); }, []);
  ...
  const handlePageChange = useCallback((i) => {
    if (i === activePage) return;
    setActivePage(i);
    setPageKey(k => k + 1);
  }, [activePage]);

  const handleSetFrameIdx = useCallback((i) => { setFrameIdx(i); setColorIdx(0); }, []);
  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          background: bgStyle,
          '--dynamic-text': contrastColor,
          '--dynamic-text-dim': contrastColor === '#ffffff' ? 'rgba(255,255,255,0.55)' : 'rgba(0,0,0,0.55)',
          '--dynamic-text-muted': contrastColor === '#ffffff' ? 'rgba(255,255,255,0.25)' : 'rgba(0,0,0,0.25)',
          '--dynamic-border': contrastColor === '#ffffff' ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.07)',
          '--dynamic-surface': contrastColor === '#ffffff' ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)',
          '--dynamic-btn-text': contrastColor === '#ffffff' ? '#111111' : '#ffffff',
          '--dynamic-btn-bg': contrastColor === '#ffffff' ? 'rgba(255,255,255,0.85)' : 'rgba(0,0,0,0.85)',
        }}
      >
