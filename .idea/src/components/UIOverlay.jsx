...
const TITLES = ['Choose your frame','Pick your material','Select your lens','Choose your colour','Select your size','Your custom pair'];
const SUBS   = [
      ...
      <div className="ui-scroll">
        ...
        <div className="step-content" key={step}>
          ...
          {step === 0 && (
            <>
              <div className="frame-list">
                {frames.map((f, i) => (
                  <div key={f.id} className={`frame-item ${frameIdx === i ? 'active' : ''}`} onClick={() => setFrameIdx(i)}>
          ...
          {step === 1 && materials.map((mt, i) => (
            <button key={mt.id} className={`opt-card ${matIdx === i ? 'selected' : ''}`} onClick={() => setMatIdx(i)}>
          ...
          {step === 2 && lensTypes.map((lt, i) => (
            <button key={lt.id} className={`opt-card ${lensIdx === i ? 'selected' : ''}`} onClick={() => setLensIdx(i)}>
              <div className="opt-card-top">
              ...
              <div className="color-swatches">
                {frame.colors.map((c, i) => (
                  <div key={c.name} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:6 }}>
                    <button className={`color-swatch ${colorIdx === i ? 'selected' : ''}`} style={{ background: c.hex }} onClick={() => setColorIdx(i)} />
                    <span style={{ fontSize:10, opacity:0.4 }}>{c.name}</span>
                  </div>
                ))}
              </div>
              <p style={{ fontSize:12, opacity:0.35, lineHeight:1.6 }}>Pigment is mixed into the filament before printing.</p>
            </>
          )}
          ...
          {step === 4 && (
            <div className="size-grid">
              {sizes.map((sz, i) => (
                <button key={sz.id} className={`opt-card ${sizeIdx === i ? 'selected' : ''}`} onClick={() => setSizeIdx(i)}>
                  <div className="opt-card-top">
          ...
          {step === 5 && (
            <>
              ...
              <div className="eco-box">
                ...
                <div className="eco-box-text">Your pair uses approximately 15g of recycled plastic, diverting ~12 bottle caps from landfill. {material.co2}.</div>
              </div>
              <div className="total-row">
                <span className="total-label">Total</span>
                <span className="total-price">₱{totalPrice.toLocaleString()}</span>
              </div>
              <button className="btn-order" onClick={() => displayToast('Preparing checkout…')}>Order Custom Pair</button>
              <button
                onClick={() => {
                  const txt = `${frame.name} | ${material.name} | ${lens.name} | ${color.name} | ${size.name} | ₱${totalPrice}`;
                  copyToClipboard(txt).then(() => displayToast('Specs copied')).catch(() => displayToast('Copy failed'));
                }}
                style={{ background:'none', border:'none', color:'rgba(255,255,255,0.3)', fontSize:11, cursor:'pointer', letterSpacing:1, textTransform:'uppercase', fontFamily:'var(--font-body)', padding:'4px 0' }}
              >
                Copy Specs
              </button>
            </>
          ...
          <div className="step-nav">
            {step > 0 && <button className="btn-back" onClick={prevStep}>Back</button>}
            {step < STEPS.length - 1 && <button className="btn-next" onClick={nextStep}>Next: {STEPS[step + 1]}</button>}
          </div>
